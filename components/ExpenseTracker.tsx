...
import { Transaction, ExpenseCategory, BankAccount, CreditCard, Wallet, Loan } from '../types';
import { formatCurrency } from '../utils/calculations';
import { Plus, Trash2, Edit2, XCircle, ShoppingBag, Wallet as WalletIcon, Filter, Settings, Save, Check, X, Link as LinkIcon, Info } from 'lucide-react';
...
const ExpenseTracker: React.FC<ExpenseTrackerProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    sourceId: accounts[0]?.id || creditCards[0]?.id || wallets[0]?.id || '',
    loanId: ''
  });
  ...
  const filteredTransactions = useMemo(() => {
    return transactions
      .filter(t => t.type === 'expense' && t.category === activeTab && (sourceFilter === 'all' || t.sourceId === sourceFilter))
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-20">
      <ConfirmModal 
        ...
        message="This will restore the funds back to your linked source."
      />
        ...
        {(['Personal', 'Business'] as ExpenseCategory[]).map(cat => (
          <button
            ...
            className={`flex-1 py-3 rounded-xl text-xs font-black transition-all uppercase tracking-wider ${
              activeTab === cat ? 'bg-white dark:bg-slate-800 text-rose-600 dark:text-rose-400 shadow-md' : 'text-slate-500 dark:text-slate-500'
            }`}
          >
            {cat}
          </button>
      ...
      {!showAddForm && (
        <div className="flex gap-3">
          <button 
            ...
            className="flex-1 bg-rose-600 text-white p-5 rounded-[2rem] font-black uppercase tracking-widest flex items-center justify-center gap-3 shadow-xl shadow-rose-500/20 active:scale-95 transition-all"
          >
      ...
      {showAddForm && (
        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl animate-in fade-in zoom-in-95">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-black uppercase tracking-tighter flex items-center gap-2">
              <ShoppingBag size={20} className="text-rose-500" /> {editingTransaction ? 'Edit Expense' : 'Log Expense'}
            </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
               <div className="col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 text-sm font-bold dark:text-white focus:ring-2 ring-rose-500"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 text-sm font-black dark:text-white focus:ring-2 ring-rose-500"
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-5 py-4 text-xs font-bold dark:text-white focus:ring-2 ring-rose-500"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-2">Payment Source</label>
                <select
                  required
                ...
                >
                  ...
                  <optgroup label="Credit Cards">
                    {creditCards.map(card => <option key={card.id} value={card.id}>{card.nickname || card.name}</option>)}
                  </optgroup>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-2 flex items-center gap-1.5">
                  Link to Loan (EMI Payoff)
                </label>
                <select
                ...
                {formData.loanId && (
                  <p className="mt-1.5 flex items-center gap-1.5 text-[9px] font-black text-blue-500 uppercase tracking-widest ml-2">
                    <Info size={10} /> This will automatically update your loan balance.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="relative">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-2">Expense Category</label>
            ...
            <button type="submit" className="w-full bg-slate-900 dark:bg-rose-600 text-white py-5 rounded-[1.5rem] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">
      ...
      <div className="bg-slate-900 p-6 rounded-[2rem] text-white flex justify-between items-center shadow-lg shadow-slate-500/10">
         <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50 mb-1">Total {activeTab} Outflow</p>
            <p className="text-3xl font-black tracking-tight text-rose-400">{formatCurrency(filteredTransactions.reduce((acc, t) => acc + t.amount, 0))}</p>
         </div>
          ...
          filteredTransactions.map(tx => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-rose-50 dark:bg-rose-900/10 text-rose-600 rounded-2xl flex items-center justify-center">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-0.5">
                    ...
                    {tx.loanId && (
                      <div className="flex items-center gap-1 px-1.5 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-md text-[8px] font-black uppercase tracking-wider">
                        <LinkIcon size={8} /> Loan: {getLoanName(tx.loanId)}
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 p-8 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
        ...
        <form onSubmit={handleAdd} className="flex gap-2 mb-6">
          <input 
            ...
            className="flex-1 bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-sm font-bold dark:text-white focus:ring-2 ring-rose-500"
          />
          <button type="submit" className="p-3 bg-rose-600 text-white rounded-xl shadow-lg hover:bg-rose-700 transition-all">
            <Plus size={20} />
          </button>
        </form>
          ...
          {categories.map((cat, idx) => (
            <div key={cat} className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl flex items-center justify-between group">
              {editingIndex === idx ? (
                ...
                <>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-white dark:bg-slate-800 rounded-lg flex items-center justify-center text-rose-500">
                      {getCategoryIcon(cat)}
                    </div>
